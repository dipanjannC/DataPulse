...
## Text2SQL stack — the active NL-to-SQL app

The natural-language-to-SQL application is organized across `src/` (backend), `text2sql/agent/` (the NL→SQL agent), and `frontend/` (static UI): a
...
Security data. Deploys via `render.yaml` (`uvicorn src.api.main:app`) and is
...
```bash
uv run python -m src.pipeline                 # generate → validate → SQLite + Neo4j KG
uv run uvicorn src.api.main:app --port 8000   # then open http://localhost:8000/
```

> The `src/` sections below describe an earlier graph-querying stack and are partly out of
> date (the NetworkX/langchain query modes were removed). New work happens in the codebase.
